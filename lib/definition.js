...
const transform = (def, options) => {
  ...
  const formatAliases = def =>
    def.update("aliases", aliases => {
      ...
      aliases.map(
        (alias, key) => {
          if(Immutable.Map.isMap(alias)){
            const iteration = mapIterator(key, alias);
            for(let i = 0; i < iteration.length; i++){
              updatedAliases = updatedAliases.set(iteration[i][0], iteration[i][1]);
            }
          }
        });

      return updatedAliases.map(
        alias =>
    ...
    def
      .update("aliases", aliases =>
        def
          .get("imports")
          .reduce(
            (aliases, i) => aliases.merge(i.get("aliases", Immutable.OrderedMap())),
  ...
  /* hmalaud flavor */
  const mapIterator = (fkey, map, aliases = []) => {
    if(!Immutable.Map.isMap(map)){
      aliases.push([fkey, map]);
    }
    else{
      if( fkey == 'mq' && typeof map.get('value') !== 'undefined'){
        aliases.push([fkey, map]);
      }
      map.map( (v, k) => {
        const name = k == 'value' ? fkey : `${fkey}-${k}`;
        mapIterator(name, v, aliases);
      });
    }
    return aliases;
  }
  ...
  /* hmalaud flavor */
  const objIterator = (fkey, obj) => {
    let aliases = [];
    for( const key in obj ){
      const name = `${fkey}${fkey.indexOf('-'+key) !== -1 ? '' : '-'+key}`;
      if( typeof obj[ key ] != 'object' ){
        aliases.push(name);
      }
      else{
        const iteration = objIterator( name, obj[ key ] );
        aliases.push(fkey, obj);
        for( let i = 0; i < iteration.length; i++ ){
          aliases.push(iteration[i]);
        }
      }
    }
    return aliases;
  ...
  /* hmalaud flavor */
  // const applyMultipleAliases = (aliases, v) => {
  ...
  /* hmalaud flavor */
  const applyAliases = (aliases, v) =>
    allMatches(v, ALIAS_PATTERN).reduce((v, [alias, key]) => {
      if (!aliases.has(key))
        throw new Error(`Alias "${key}" not found`);
      const val = typeof aliases.getIn([key, "value"]) == 'object' ? aliases.getIn([key, "value"]).toJS() : aliases.getIn([key, "value"]);
      ...
      return props.map((value, key) => {
        return value.update("value", v => {
          return recursiveMap(v, v => {
            ...
            v = /Map /.test(v) ? v.replace(/Map /g,'') : v;
            return applyAliases(aliases, v);
          });
        });
      });
    });
